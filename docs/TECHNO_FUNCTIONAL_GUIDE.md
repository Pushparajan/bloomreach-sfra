...
| `ThematicPage-PersonalizedStrip` | `GET /ThematicPage-PersonalizedStrip?jobType=<v>&toeShape=<v>&safetySpec=<v>` | No-cache (R-38) | Fetched client-side by a generated Thematic Page's own body; "Recommended for You" fragment or nothing (see §5.4, §5.6) |
...
- **`interactiveCache.applyNoCache`** — applied to Boot Finder, Compare, Work-PersonalizedStrip, Product-PersonalizedStrip, Search-PersonalizedRail, and ThematicPage-PersonalizedStrip (R-38). Forces a fresh server call every time. Ensures personalized state (answers, selections, 1:1-personalized content) is never served from a shared cache.
- **`cache.applyDefaultCache`** — applied to Work-JobLanding's shell. Allows SFCC's standard page cache to absorb repeat traffic since the shell's own content is not shopper-specific. `Product-Show` and `Search-Show`'s own cache policy is whatever the base cartridge already configures - this integration only appends data via `server.extend`/`server.append` and never touches their caching. Thematic Pages aren't a live route at all - they're Content assets regenerated by a nightly batch job (§5.4), effectively cached for up to 24h between runs.
...
**Scope - six call sites, one exclusion pattern:**
...
| `GenerateThematicPages` job / the generated page's own static grid | No, never | The batch job has no shopper/request context; never calls `resolveShopperIdentity` |
| `ThematicPage-PersonalizedStrip` **fragment** (§5.6) | Yes, when eligible | Same pattern, within a separate, uncached fragment fetched from the generated page's own body |
...
### 5.6 PDP, Category & Thematic Page 1:1 Personalization (R-38)

**Business goal:** extend the same shell/fragment 1:1-personalization pattern built for Work Job Landing onto every other page type in this integration - the Product Detail Page (PDP) and Category/PLP browse pages (both owned by the base cartridge), and the batch-generated Thematic Pages (owned entirely by this cartridge).

**Architecture:** identical principle everywhere - a cached/static shell (untouched) plus a separate, uncached fragment fetched client-side, carrying `user_id` only when eligible (§4.7). No fragment ever re-ranks or replaces the shell's own content; each only **adds** a personalized section. Full-grid/full-PDP personalization is explicitly out of scope for this pattern - same boundary already drawn around free-text Search/Autosuggest - it would need a cache-variant redesign or the Next.js end state.

| Page | Shell (untouched) | Fragment |
|---|---|---|
| PDP | Images, price, size/color, reviews, SEO (base cartridge) | `Product-PersonalizedStrip` - "Recommended With This" |
...
**Thematic Page recommendation basis:** unlike the PDP, the Thematic Page fragment reuses the exact hard-filtered `job_type`/`toe_shape`/`safety_specs` shape the page's own generation already used (topically consistent with the page's theme), plus `user_id` - so personalization shows up in which products Bloomreach ranks highest for that combination, not in loosening the page's own topic.
