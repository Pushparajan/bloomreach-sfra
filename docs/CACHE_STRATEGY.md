...
### 3.1 Thematic pages as cached compare entry points

Although `Compare-Show` itself is never cached, thematic pages generated by the nightly `GenerateThematicPages` batch job embed compare controls directly in their content asset body and therefore act as a **cached entry point** into the comparison feature.

#### What the batch job writes into each thematic content asset

`GenerateThematicPages.js` writes two blocks back-to-back into `content.custom.body`:

1. A `<script type="application/ld+json">` block containing schema.org `ItemList` structured data.
2. A product grid of `<div class="product-tile thematic-page__tile">` elements, each containing:
   - A `<input type="checkbox" data-compare-select data-vg-id="..." data-name="..." data-image="...">` control that hooks into `compare.js`'s existing delegated handlers.
   - A **View Comparison** `<button data-compare-view data-url=".../Compare-Show">` that triggers the existing `Compare-Show` route when the shopper has selected ≥ 2 products.

No new client JavaScript or controller is introduced; the entire compare flow reuses the existing `compare.js` and `Compare-Show` contract unmodified.

#### Cache boundary

| Layer | Cache policy | Who controls it |
|---|---|---|
| Thematic content asset page (the landing page served by the platform's `Content-Show` controller) | Standard SFCC page cache (`cache.applyDefaultCache`) — same TTL as PLPs/PDPs | Platform controller; not this integration |
| `Compare-Show` (invoked when the shopper clicks "View Comparison") | **No cache** (`interactiveCache.applyNoCache`) | `Compare.js` (this integration) |

The two-phase flow is:

```
Shopper lands on thematic page (CACHED)
  → selects products via compare checkboxes (client-side sessionStorage only)
    → clicks "View Comparison"
      → browser requests Compare-Show?pids=... (NO CACHE — fresh Bloomreach call)
```

#### Staleness risk from thematic page cache

Because the product grid is baked into the content asset at batch-job time, a cached thematic page may show compare checkboxes for a product line-up that has changed since the previous nightly run:

| Risk | Mitigation |
|---|---|
| Thematic page HTML is cached with a product grid from the previous nightly run | Standard page cache TTL expiry. For immediate refresh, manually invalidate the thematic page URL in BM (**Administration > Sites > [Site] > Cache Settings > Invalidate**) after the nightly job completes. |
| A product in the thematic grid is out of stock but the compare checkbox is still shown | The `Compare-Show` call is always live (no-cache) and fetches current Bloomreach data; the comparison table will reflect real-time availability even if the entry-point page is stale. |
| `SAFETY_SPEC_REFINEMENT` flag is toggled off mid-day after a cached thematic page was already generated | Thematic pages with a `safetySpec` combination are skipped by the next job run (`isEligible` gate), but previously cached HTML remains until TTL expiry. Trigger manual invalidation to remove immediately. |

#### Cache key for thematic compare pages

Thematic pages are keyed by their content asset URL (e.g. `/work-thematic-pages/electrical-steel-toe`). Each combination is an independent cache entry. The compare controls embedded in the page body do not affect the cache key — they are part of the static HTML rendered at cache-fill time.
...
| **Product Comparison — thematic entry point** | Thematic content asset page (no dedicated HTTP route; served by platform `Content-Show`) | `cache.applyDefaultCache` (platform controller) | Site default (e.g. 15–30 min) | Product grid with compare controls baked in at batch-job time; URL-deterministic |
...
| Product Comparison — thematic entry point | `public, max-age=<site-ttl-in-seconds>` (platform default; compare controls baked into cached HTML) |
...
Because Boot Finder and `Compare-Show` use no-cache (`cachePeriod = 0`), feature flag changes take effect **on the next browser request** with no action required. There is no cached page to invalidate.

**Exception — thematic compare entry pages:** Thematic pages that embed compare checkboxes are served from the standard page cache. If you toggle a feature flag (e.g. `SAFETY_SPEC_REFINEMENT`) that affects which thematic combinations are generated:

1. Re-run `GenerateThematicPages` (or wait for the nightly job) so content assets are updated.
2. Manually invalidate the affected thematic page URLs in BM (**Administration > Sites > [Site] > Cache Settings > Invalidate**) to ensure shoppers see the refreshed product grid with up-to-date compare controls.

The `Compare-Show` step in the flow (triggered when the shopper clicks "View Comparison") always runs fresh regardless of the thematic page cache state.
...
| Product Comparison (`Compare-Show`) | Same as Boot Finder — no CDN caching. |
| Product Comparison — thematic entry point | CDN may cache per its own TTL rules (page is `public`). Ensure CDN purge is triggered after the nightly `GenerateThematicPages` job completes and after any manual BM cache invalidation. |
